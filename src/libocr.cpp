...
libocr::ocr(const char *image_data, int image_data_size, char *result, int result_size) {
    return ocr_file_data(image_data, image_data_size, result, result_size);
}

int _stdcall

libocr::ocr_image_data(int image_width, int image_height, const char *image_data, int image_data_size, char *result,
                       int result_size) {
...
libocr::ocr_file_path(const char *image_file, char *result, int result_size) {
...
libocr::ocr_file(const char *file_path, int *file_size) {
...
libocr::ocr_image(int image_width, int image_height, const char *image_data, int image_data_size) {
    char *buffer = new char[1024];
